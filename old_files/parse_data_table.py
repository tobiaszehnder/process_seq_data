...
def main():
    ...
    if not len(sys.argv) == 3:
        print('Usage: prun mdl parse_data_table.py project_dir data_table')
        sys.exit(0)
    project_dir, data_table = [os.path.abspath(x) for x in sys.argv[1:]]
        
    # load modules
    import numpy as np, pandas as pd, urllib.request, collections, glob, re
    ...
    if mpi.shape[0] > 0:
        ...
        for idx,row in mpi.iterrows():
            ...
            links = ['%s/fastq/%s_%s_%s.fastq.gz' %(row['data_dir'], row['fastq_sample'], row['flow_cell'], r) for r in row['reads']]
        ...
        mpi = mpi.loc[:,['sample','fastq_sample','experiment','sequencing_type', 'species']]
    else:
        mpi = pd.DataFrame()
    ...
    if geo.shape[0] > 0:
        ...
        geo = geo.loc[:,['sample','fastq_sample','experiment','sequencing_type','species']]
    else:
        geo = pd.DataFrame()
    ...
    if enc.shape[0] > 0:
        ...
        enc = enc.loc[:,['sample','fastq_sample','experiment','sequencing_type','species']]
    else:
        enc = pd.DataFrame()
    
    # --------------------------
    ## concat and write data
    outfile  = re.sub('.csv', '.df', data_table)
    df = pd.concat([mpi, geo, enc], axis=0).sort_index() #.to_csv(outfile, index=False, header=False)
